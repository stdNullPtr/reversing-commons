...
{
    inline std::optional<DWORD> GetProcessIdByName(const std::wstring& processName)
    {
    ...
    inline std::optional<HANDLE> OpenProcessHandle(const std::wstring& processName)
    {
        const auto processId{GetProcessIdByName(processName)};
        if (!processId)
        {
            std::wcout << XORW(L"Process not found: ") << processName << '\n';
            return std::nullopt;
        }
                                                             ...
                                                             std::chrono::seconds(5))
    {
        ...
        for (int i{1}; i <= maxRetries; ++i)
        {
            ...
            if (i < maxRetries)
            {
                std::wcout << XORW(L"Will try again after ") << retryDelay << " seconds \n";
